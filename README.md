...
コピー先の対応ファイルシステムは NTFS、ReFS、exFAT、FAT、FAT32 です。取込済判定とコピー後検証では、サイズ一致に加えて、NTFS/ReFS は完全一致、exFAT は 10ms 単位、FAT/FAT32 はローカル時刻の 2 秒単位で更新日時を照合します。未知のファイルシステムではコピーを開始しません。
...
- コピー先ファイルシステムの時刻精度を取込済判定、一時ファイル検証、正式ファイル検証で一貫して使用します。
- カメラでプロテクトされた読み取り専用ファイルは、その属性を保ったままコピーします。キャンセルや失敗時は、この処理が作成し安全条件を満たす一時ファイルだけ ReadOnly を解除して削除します。
- 読み取り専用の既存ファイルも上書きできます。確定失敗時は旧ファイルの ReadOnly を復元・再確認し、確認できない場合は復旧対象のパスをエラーに表示します。
