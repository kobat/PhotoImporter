...
ファイルシステム情報とExif情報を使うテンプレートに対応しています。Exifキャッシュは通常スキャンへ接続済みで、利用ON/OFFと保存先を設定できます。カード管理画面、多言語化、一覧フィルター、単一exe配布は未実装です。
...
- UIは現在、日本語のみです。
- [FILTER_SPEC.md](FILTER_SPEC.md) で仕様化した、ファイル種別・拡張子・ファイルシステム/Exifトークン・状態による一覧フィルターは未実装です。
