...
ファイルシステム情報とExif情報を使うテンプレートに対応しています。Exifキャッシュは通常スキャンへ接続済みで、利用ON/OFFと保存先を設定できます。一覧フィルターは型付き条件、表示・選択連携、条件編集UI、条件適用時の遅延Exif読込まで実装済みです。カード管理画面、多言語化、単一exe配布は未実装です。
...
2026-07-25時点では、285件の単体テストがRelease構成ですべて成功しています。
...
- UIは現在、日本語のみです。
- [FILTER_SPEC.md](FILTER_SPEC.md) で仕様化した一覧フィルターは、第1～第3段階を実装済みです。条件と選択解除設定の永続化、および実機写真を使った形式別のUI連携確認は未実装・継続確認項目です。
- 配布用の単一exe化は未実装です。
